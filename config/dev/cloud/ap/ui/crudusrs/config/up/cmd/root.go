...
to quickly create a Cobra application.`,
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		abbr, err := cmd.Flags().GetString("l")
		if err != nil {
			panic(err)
		}
		...
		command := fmt.Sprintf("sudo cp /home/nate/code/app-pathway/config/up/dockerignore/%s .dockerignore", srv.name)
		run(command, wd)
...
func init() {
	...
	rootCmd.PersistentFlags().StringVar(&cfgFile, "config", "", "config file (default is $HOME/.up.yaml)")
	rootCmd.Flags().String("l", "", "logs")
	rootCmd.Flags().Bool("b", false, "build")
