...
import (
	...
	"apppathway.com/pkg/builder/base/dgraph"
	"apppathway.com/pkg/builder/base/net/grpc"
	"apppathway.com/pkg/builder/base/ristretto"
...
func (m *Main) Run() (err error) {
	...
	// a more robust service if we expanded out to multiple base.
	cacheService := ristretto.NewCacheService[dgraph.Schema]()

	// Attach our cache service to the dgraph database.
	m.DB.Cache = cacheService

	// Instantiate dgraph-backed services.
	genService := dgraph.NewCPluginService(m.DB)

	// Attach gen service to Main for testing.
	m.GenService = genService

	// Copy configuration settings to the GRPC server.
	m.GRPCServer.Addr = m.GRPC.Addr

	// Attach underlying services to the GRPC server.
	m.GRPCServer.CPluginService = genService
...
type Main struct {
	...
	GenService base.CPluginService
}
...
func NewMain() (*Main, error) {
	...
	return &Main{
		Config:     conf,
		DB:         dgraph.NewDB(conf.DB.DSN, conf.DB.TLS),
...
func DefaultConfig() (Config, error) {
	...
	conf = Config{
		...
		GRPC: struct {
			...
			Addr: os.Getenv("CPLUGIND_ADDRESS"),
			TLS: &tls.Config{
