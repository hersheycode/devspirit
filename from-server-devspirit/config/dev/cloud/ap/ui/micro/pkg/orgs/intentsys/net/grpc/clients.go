...
func NewIntentClient(connStr, caFilePath string) *IntentClient {
	...
	return &IntentClient{IntentClient: intentpb.NewIntentClient(connection), conn: connection}
}
func NewSchedulerClient(connStr, caFilePath string) *SchedulerClient {
	connection := tools.Connect(connStr, caFilePath)
	return &SchedulerClient{SchedulerClient: schedulerpb.NewSchedulerClient(connection), conn: connection}
}
func NewSMSClient(connStr, caFilePath string) *SMSClient {
	connection := tools.Connect(connStr, caFilePath)
	return &SMSClient{SMSClient: smspb.NewSMSClient(connection), conn: connection}
}
func NewIntentSysClient(connStr, caFilePath string) *IntentSysClient {
	fmt.Println("Creating client for intentsysd...", connStr)
	connection := tools.Connect(connStr, caFilePath)
	return &IntentSysClient{IntentSysClient: intentsyspb.NewIntentSysClient(connection), conn: connection}
}
